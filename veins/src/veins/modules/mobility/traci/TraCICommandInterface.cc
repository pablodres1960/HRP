...
//PABLO me da el # de vehiculos activos en la simulacion
...
//PABLO
double TraCICommandInterface::Vehicle::getCurrentBatteryCapacity() const
{
    return traci->genericGetDouble(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_CURRENTBATTERYCAPACITY, RESPONSE_GET_VEHICLE_VARIABLE);
}
//PABLO
double TraCICommandInterface::Vehicle::getDestination() const
{
    return traci->genericGetDouble(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_DESTINATION, RESPONSE_GET_VEHICLE_VARIABLE);
}
...
void TraCICommandInterface::Vehicle::changeTarget(std::string edgeID) const
{
    uint8_t variableId = CMD_CHANGETARGET;
    uint8_t variableType = TYPE_STRING;
    TraCIBuffer buf = connection->query(CMD_SET_VEHICLE_VARIABLE, TraCIBuffer() << variableId << nodeId << variableType << edgeID);
    ASSERT(buf.eof());
}
